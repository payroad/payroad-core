...
final class InitiateCardRefundUseCaseTest extends TestCase
{
    ...
    private InitiateCardRefundUseCase $useCase;
        ...
        $this->cardProvider
            ...
            ->willReturnCallback(
                fn(RefundId $id, PaymentId $paymentId, PaymentAttemptId $attemptId, string $providerName, Money $amount) =>
                    CardRefund::create($id, $paymentId, $attemptId, $providerName, $amount, new StubCardRefundData())
            );

        $this->useCase = new InitiateCardRefundUseCase(
    ...
    private function makeAttempt(Payment $payment): CardPaymentAttempt
    {
    ...
    public function testExecuteReturnsCardRefund(): void
    {
        ...
        $result = $this->useCase->execute(new InitiateCardRefundCommand(
            $payment->getId(),
            Money::ofMinor(500, new Currency('USD', 2))
        ));

        $this->assertInstanceOf(CardRefund::class, $result);
    }
    ...
    public function testExecuteCallsForCardOnRegistry(): void
    {
        ...
        $this->useCase->execute(new InitiateCardRefundCommand(
            $payment->getId(),
            Money::ofMinor(500, new Currency('USD', 2))
        ));
    }
    ...
    public function testExecuteSavesRefund(): void
    {
        ...
        $this->refunds
            ->expects($this->once())
            ->method('save')
            ->with($this->isInstanceOf(CardRefund::class));
    ...
    public function testExecutePassesContextToProvider(): void
    {
        $payment = $this->makeSucceededPayment();
        $attempt = $this->makeAttempt($payment);
        $context = new CardRefundContext(reason: 'duplicate');
