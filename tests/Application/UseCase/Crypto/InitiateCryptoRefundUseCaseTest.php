...
use Payroad\Application\UseCase\Crypto\InitiateCryptoRefundCommand;
use Payroad\Application\UseCase\Crypto\InitiateCryptoRefundUseCase;
...
use Tests\Stub\StubCryptoRefundData;

final class InitiateCryptoRefundUseCaseTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->useCase = new InitiateCryptoRefundUseCase(
    ...
    private function makeAttempt(Payment $payment): CryptoPaymentAttempt
    {
        $attempt = CryptoPaymentAttempt::create(
        ...
        $attempt->setProviderReference('tx_abc123');
        $attempt->releaseEvents();
        return $attempt;
    }

    private function makeContext(): CryptoRefundContext
    {
        return new CryptoRefundContext(returnAddress: '0xABC...');
    }

    public function testExecuteReturnsCryptoRefund(): void
    {
        ...
        $result = $this->useCase->execute(new InitiateCryptoRefundCommand(
            $payment->getId(),
            Money::ofMinor(500, new Currency('USD', 2)),
            $this->makeContext()
        ));

        $this->assertInstanceOf(CryptoRefund::class, $result);
    }

    public function testExecuteCallsForCryptoOnRegistry(): void
    {
        $payment = $this->makeSucceededPayment();
        $attempt = $this->makeAttempt($payment);
    ...
    public function testExecuteSavesRefund(): void
    {
        ...
        $this->refunds
            ->expects($this->once())
            ->method('save')
            ->with($this->isInstanceOf(CryptoRefund::class));
    ...
    public function testExecutePassesContextToProvider(): void
    {
        $payment = $this->makeSucceededPayment();
        $attempt = $this->makeAttempt($payment);
        $context = new CryptoRefundContext(returnAddress: '0xABC...', network: 'erc20');
